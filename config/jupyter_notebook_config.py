...
def output_post_save(model, os_path, contents_manager):
    _os_path = os_path.replace(" ", "\\ ")
    ftype = model.get('type')
    if ftype not in ["notebook", "file"]:
        print("Only process IPython Notebook or Markdown file")
        return
    fname = model.get('name')
    if 'Untitled' in fname:
        print("Newly created file will not be processed")
        return
    if ftype == "file" and fname.split(".")[-1] != "md":
        print("Markdown only")
        return
    category = os.path.split(model.get('path'))[0]
    title = fname.split(".")[0].replace(" ", "\\ ")

    dest_dir = pjoin(_BLOG_ROOT, "docs").replace(" ", "\\ ")
    if not os.path.isdir(dest_dir):
        os.mkdir(dest_dir)
    dest_name = (model.get('path').split(".")[0] + ".md").replace(" ", "\\ ")

    if ftype == "notebook":
        print("call nbconvert")
        subprocess.check_output(
            "jupyter-nbconvert --to markdown {} && mv {}.md {}".format(
                _os_path, pjoin(os.path.dirname(_os_path), title), pjoin(dest_dir, dest_name)),
            shell=True)
    else:

        if title == _DEFAULT_CATprocessEGORY:
            dest_name = "index.md"
        subprocess.check_output("cp {} {}".format(_os_path, pjoin(dest_dir,
                                                                 dest_name)),
                                shell=True)
    isCatePage = False
    if not category:
